...
namespace LoanLogic
{
    public static class LoanExcelExporter
    {
        public static void Export(Loan loan, string filePath)
        {
            ...
            // Данные
            bool stop = false;
            for (int i = 0; i < loan.TermMonths | stop; i++)
            {
                var row = i + 2;
                worksheet.Cell(row, 1).Value = (int)loan.Payouts[i, 0];
                worksheet.Cell(row, 2).Value = loan.Payouts[i, 1];
                worksheet.Cell(row, 3).Value = loan.Payouts[i, 2];
                worksheet.Cell(row, 4).Value = loan.Payouts[i, 3];
                worksheet.Cell(row, 5).Value = loan.Payouts[i, 4];
                if (loan.Payouts[i, 5] == 0)


                totalPayment += loan.Payouts[i, 1];
                totalInterest += loan.Payouts[i, 2];
            }

            // Итоги внизу
            int totalRow = loan.TermMonths + 3;
