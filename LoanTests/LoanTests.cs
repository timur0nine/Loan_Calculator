   ...
   
{
    ...
    public class LoanTests
    {
        ...
        public void CalculateAnnuityShouldFillPayouts()
        {
            ...
            Assert.IsTrue(payouts[0, 1] > 0); // первый платЄж
            Assert.IsTrue(payouts[0, 4] > 0); // остаток после первого мес€ца
        }
        ...
        public void CalculateDifferentiatedShouldFillPayouts()
        {
            ...
            Assert.AreEqual(12, payouts.GetLength(0));
            Assert.AreEqual(5, payouts.GetLength(1));
            Assert.IsTrue(payouts[0, 1] > payouts[11, 1]); // платЄж уменьшаетс€
        }
        [TestMethod]
        public void EarlyRepayment_ShouldReduceDebtFaster_Annuity()
        {
            ...
            loan.AddEarlyRepayment(2, 20000m); // ƒосрочное погашение во 2-й мес€ц

            loan.Calculate();

            decimal remainingAfter12 = loan.Payouts[11, 4];
            Assert.AreEqual(0m, remainingAfter12, "ƒолг должен быть полностью погашен до окончани€ срока из-за досрочного платежа.");
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                if (loan.Payouts[i, 4] == 0)
                {
                    ...
                    {
                        Assert.AreEqual(0m, loan.Payouts[j, 1], $"ѕлатеж в мес€це {j + 1} должен быть равен 0 после досрочного погашени€.");
                    }
                    break;
                }
            }
